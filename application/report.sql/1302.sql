...
FROM (
	SELECT 
		...
		IFNULL(st.`Country`, '') AS `Country`, IFNULL(st.`City`, '') AS `City`, IFNULL(st.`District`, '') AS `District`, IFNULL(st.`Quarter`, '') AS `Quarter`, IFNULL(st.`Avenue`, '') AS `Avenue`, IFNULL(st.`Lat`, '') AS `Lat`, IFNULL(st.`Long`, '') AS `Long`, IFNULL(st.`Page`, 0) AS `Page`, IFNULL(st.`DeviceID`, '') AS `DeviceID`, 
		getDuration(st.`CustomerID`, st.`ApplicationID`, st.`ContentID`, st.`DeviceID`, st.`Type`, st.`Page`, st.`Time`) AS `Duration` 
	...
	FROM `Customer` cu 
		...
		INNER JOIN `Statistic` st ON st.`CustomerID`=cu.`CustomerID` AND st.`ApplicationID`=ap.`ApplicationID` AND st.`ContentID`=cn.`ContentID` AND (st.`Type`='21' OR st.`Type`='22') AND st.`Time` BETWEEN '{SD}' AND '{ED}' AND (st.`Country`={COUNTRY} OR {COUNTRY} IS NULL) AND (st.`City`={CITY} OR {CITY} IS NULL) AND (st.`District`={DISTRICT} OR {DISTRICT} IS NULL)
	WHERE 
