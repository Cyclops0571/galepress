...
@section('content')
    ...
    $authMaxPDF = Common::AuthMaxPDF($app->ApplicationID); //571571 This should be checked at the server side...
    ?>
    ...
    <div class="col-md-9" style="padding-left:25px;">
      <div class="block">
        ...
        <div class="content controls" style="overflow:visible">
          ...
          <div class="form-row">
            ...
            <div class="col-md-8">
              <textarea class="form-control" name="Detail" id="Detail" rows="2"
                        cols="15">{{ $content->Detail }}</textarea>
            </div>
            ...
            <div class="col-md-8">
              <select id="CategoryID" name="chkCategoryID[]" multiple="multiple" class="chosen-container"
                  ...
                  if ($content->ContentID) {
                      $cnt = ContentCategory::where('ContentID', '=', $content->ContentID)->where('CategoryID', '=', '0')->count();
                  }
                  ?>
                @if($cnt>0)
                ...
                @endif
                  ...
                  if ($content->ContentID) {
                      $cnt = ContentCategory::where('ContentID', '=', $content->ContentID)->where('CategoryID', '=', $category->CategoryID)->count();
                  }
                  ?>
                <option
                    value="{{ $category->CategoryID }}"{{ ($cnt > 0 ? ' selected="selected"' : '') }}>{{ $category->Name }}</option>
                  <?php endforeach; ?>
              </select>
            </div>
          ...
          <div class="form-row">
            <div
                class="col-md-3">{{ __('common.contents_file') }}{{ !$content->ContentID ? ' <span class="error">*</span>' : '' }}</div>
            <div class="col-md-3" id="contentPdfFile">
              ...
              <input type="hidden" name="hdnFileName"
                     id="hdnFileName"{{ !$content->ContentID ? ' class="required"' : '' }} />
            </div>
            <div class="col-md-5" style="padding-top:2px;">
              @if($content->ContentID && $ContentFile && $authInteractivity && $Transferable)
      ...
      <div class="block">
        <div class="content controls" style="overflow:visible">
          <div class="form-row <?php echo ($content->ContentID && $IsBuyable) > 0 ? "" : 'disabledFields hide' ?>">
            <div class="col-md-3">
            ...
            <div class="col-md-8">
              <div class="input-group">
                <input type="text" id="ContentIdentifier" value="<?php echo $content->getIdentifier(); ?>"
                       readonly="readonly">
                <span class="input-group-btn">
							<button class="btn btn-primary urlCheck" type="button"
                      onclick="cContent.refreshIdentifier(<?php echo $content->ContentID; ?>);">
            ...
            <div class="form-row" id="areaCoverImg" style="text-align:center;">
              <a href="#dialog-cover-image" data-toggle="modal">
                <img class="coverImage" id="imgPreview"
                     src="/{{ Session::get('language') }}/{{ __('route.contents_request') }}?RequestTypeID=<?php echo eRequestType::NORMAL_IMAGE_FILE ?>&ContentID={{ $content->ContentID }}&W=768&H=1024"
                     width="200"/>
              </a>
            </div>
              <?php if($content->ContentID): ?>
    ...
    <div class="modal modal-info" id="modal_default_11" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-default btn-clean" data-dismiss="modal"
                    onclick="cContent.removeFromMobile(<?php echo $content->ContentID; ?>);"
            ...
            <div class="modal-content">
              ...
              <div id="coverImageModalBody" class="modal-body">
                <iframe id="coverImageIframe"
                        src="<?php echo '/' . Config::get('application.language') . '/crop/image?contentID=' . $content->ContentID ?>"
