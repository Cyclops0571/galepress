...
<div id="prop-{id}" class="component tooltip hide" componentid="{id}">
  ...
  <div class="component-header">
    ...
    <a href="javascript:void(0);" class="delete remove" title="{{ __('interactivity.delete') }}"><i
  ...
  <div class="upload">
    ...
    <div class="radiogroup">
      <div class="radio {{ ($option == 1 ? 'checked ' : '') }}js-radio"
           optionvalue="1">{{ __('interactivity.tooltip_icon_default') }}</div>
      <div class="radio {{ ($option == 2 ? 'checked ' : '') }}js-radio"
           optionvalue="2">{{ __('interactivity.tooltip_icon_new') }}</div>
    ...
    <div class="upload-icon1{{ ($fileSelected == 0 || $option == 1 ? ' hide' : '') }}">
      ...
      <!-- end progress -->
      <a {{ (Str::length($filename) > 0 ? 'href="'.$filename.'" ' : '') }} target="_blank"
         class="tooltip-icon1{{ ($fileSelected == 0 ? ' hide' : '') }}">1) {{ Str::limit($filename, 17) }}</a>
    ...
    <div class="upload-icon2{{ ($fileSelected2 == 0 || $option == 1 ? ' hide' : '') }}">
      ...
      <!-- end progress -->
      <a {{ (Str::length($filename2) > 0 ? 'href="'.$filename2.'" ' : '') }} target="_blank"
         class="tooltip-icon2{{ ($fileSelected2 == 0 ? ' hide' : '') }}">2) {{ Str::limit($filename2, 17) }}</a>
      <a class="close{{ ($fileSelected2 == 0 ? ' hide' : '') }}"><i class="icon-remove"></i></a>

      <span class="error hide"><i class="icon-exclamation-sign"></i>{{ __('interactivity.video_poster_error') }}</span>

      <div class="coordinates">
        <a href="javascript:void(0);" class="expand">{{ __('interactivity.tooltip_icon_name') }}<i class="icon-"></i>
        </a>
  ...
  <div class="settings">
    <div class="component-panel">
      ...
      <a href="javascript:void(0);" class="btn expand edit">{{ __('interactivity.tooltip_modify') }} <i
            class="icon-edit"></i></a>
    </div>
    <!-- end component-panel -->
  ...
  <!--  end settings -->
  <a href="javascript:void(0);" class="btn delete expand remove">{{ __('interactivity.remove') }} <i
