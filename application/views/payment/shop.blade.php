		...
		<div class="container">
			...
			<div class="pricing-table">
				...
				<div class="plan most-popular">
				    ...
				    <ul>
					<?php if ($priceDiffers): ?>
    					<li><b>Fiyatlar mobil uygulamaya göre değişmektedir</b> </li>
					<?php endif; ?>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
			    <?php if (count($applications) > 1): ?>
    				...
    				<label for="applicationID" class="control-label col-xs-3" style="padding-top: 16px;">Uygulama Seçimi</label>
    				<div class="col-xs-9">
    				    <select id="applicationID" onchange="setUIPrice();" class="form-control required" name="applicationID" tabindex="{{$tabIndex++}}" required>
    					<option data-price='0.00' selected="selected" >Uygulama Seçiniz</option>
					    <?php foreach ($applications as $application): ?>
						<option data-application='<?php echo json_encode($application, TRUE); ?>'  value="{{$application->ApplicationID}}" <?php echo $application->ApplicationID == $paymentAccount->ApplicationID ? 'selected="selected"' : ''; ?> >{{$application->Name}}</option>
					    <?php endforeach; ?>
    				    </select>
    				</div>
    			    </div>
    				...
    				<div class="col-md-9">
    					...
    					<input type="text" name="Price" id="Price" class="form-control textbox disabled" value="{{ $selectedApplicationPrice }}"/>
