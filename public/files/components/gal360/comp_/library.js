...
}return AC.Detector.isiPad()||AC.Detector.isMobile()||a}function warnTransform3D(){var b,f=this.showWarning=function(){if(!b){b=document.getElementById("warning");
var h=b.querySelector("a.close");h.addEventListener("click",g,false)}b.style.display="block"
},g=this.hideWarning=function(h){if(h){if(h.preventDefault){h.preventDefault()}else{h.returnValue=false
}}if(b){b.style.display=""}};var e,d=this.showWarningChrome=function(h){if(!e){e=document.getElementById("warning-chrome");
var i=e.querySelector("a.close");i.addEventListener("click",c,false)}e.style.display="block";
h.stop()},c=this.hideWarningChrome=function(h){if(h){if(h.preventDefault){h.preventDefault()
}else{h.returnValue=false}}if(e){e.style.display=""}};if(AC.Detector.isChrome()){this.links=document.querySelectorAll(".requires3d a");
for(var a=this.links.length-1;a>=0;a--){this.links[a].addEventListener("click",d,true)
}if(isTransform3DAvailable()===true){for(var a=this.links.length-1;a>=0;a--){this.links[a].removeEventListener("click",d,true)
}}}if(!(AC.Detector.isChrome())){this.links=document.querySelectorAll(".requires3d a");
for(var a=this.links.length-1;a>=0;a--){this.links[a].addEventListener("click",f,true)
}}}function recycleObjectValueForKey(a,b){var c=a[b];if(c&&typeof c.recycle==="function"){c.recycle()
}delete a[b];a[b]=null;c=null}function constrainNumberWithin(c,b,a){return(c<b)?b:((c>a)?a:c)
...
}if(b&&b.style){b.style.webkitAnimationName=a}}function setAnimation(a,b){if(typeof(a)=="string"){a=document.getElementById(a)
}if(a&&a.style){a.style.animationName=b}}(function(){var d;var c=function(){if(typeof(d)==="undefined"){if(!AC.Detector.isWebKit()){d=false
}else{if(AC.Detector.isChrome()){var h=navigator.userAgent.toLowerCase();var g=parseInt(parseFloat(h.substring(h.lastIndexOf("chrome/")+7)),10);
d=(g>=5)}else{var h=navigator.userAgent.toLowerCase();var g=parseInt(parseFloat(h.substring(h.lastIndexOf("safari/")+7)),10);
d=(g>=528)}}}return d};var f;var a=function(){if(typeof(f)==="undefined"){if(!AC.Detector.isFirefox()){f=false
...
}else{h.returnValue=false}var i=Apple.slider.activeSlider;var a=false;var c=(window.getComputedStyle)?window.getComputedStyle(i._thumb,null):i._thumb.style;
...
if(a){i._onSlide()}},onTrackClicked:function(e){if(e.preventDefault){e.preventDefault()
}else{e.returnValue=false}e.defaultPrevented=true;if(e.stopPropagation){e.stopPropagation()
}else{e.cancelBubble}var a=(Apple.browser.FF)?e.layerX:e.offsetX;var f=(Apple.browser.FF)?e.layerY:e.offsetY;
...
}else{b.returnValue=false}var a=Apple.slider.activeSlider;a._onSlideComplete()},startSlide:function(b){var a;
if(b.preventDefault){b.preventDefault()}else{b.returnValue=false}b.defaultPrevented=true;
if(b.stopPropagation){b.stopPropagation()}else{b.cancelBubble}if(Apple.browser.IE){a=Apple.slider.activeSlider=b.srcElement.sliderParent;
...
a._oldY=b.clientY;a._over=false;if(b.preventDefault){b.preventDefault()}else{b.returnValue=false
}Apple.utilities.disableTextSelection()},stopSlide:function(a){if(a.preventDefault){a.preventDefault()
}else{a.returnValue=false}a.defaultPrevented=true;if(a.stopPropagation){a.stopPropagation()
}else{a.cancelBubble}Apple.slider.onSlide(a);Apple.utilities.enableTextSelection();
Apple.slider.onSlideComplete(a);if(Apple.browser.IE){document.detachEvent("onmouseup",Apple.slider.stopSlide);
document.detachEvent("onmousemove",Apple.slider.onSlide);Apple.slider.activeSlider._track.attachEvent("onclick",Apple.slider.onTrackClicked,false)
...
}Apple.slider.activeSlider=null}};function informBrowser(){if(AC.Detector.isChrome()){var a=document.querySelectorAll(".browser");
for(var b=a.length-1;b>=0;b--){a[b].innerHTML="Chrome"}}else{if(AC.Detector.isFirefox()){var a=document.querySelectorAll(".browser");
for(var b=a.length-1;b>=0;b--){a[b].innerHTML="Firefox"}}else{if(AC.Detector.isWebKit()){var a=document.querySelectorAll(".browser");
for(var b=a.length-1;b>=0;b--){a[b].innerHTML="Safari"}}else{if(AC.Detector.isIEStrict()){var a=document.querySelectorAll(".browser");
for(var b=a.length-1;b>=0;b--){a[b].innerHTML="Internet Explorer"}}}}}}Apple.utilities={savedValueOf:new Object(),disableTextSelection:function(){switch(true){case (typeof document.onselectstart!="undefined"):this.savedValueOf.onselectstart=document.onselectstart;
...
f.target.dispatchEvent(c);if(c.defaultPrevented){if(d.preventDefault){d.preventDefault()
}else{d.returnValue=false}}if(a._previousEvent&&a._previousEvent.touches){}if(d.type==="touchend"&&!c.defaultPrevented&&a._previousEvent&&a._previousEvent.touches&&a._previousEvent.touches[0].target===f.target&&a._previousEvent.type!=="touchmove"){if(d.preventDefault){d.preventDefault()
}else{d.returnValue=false}c=document.createEvent("MouseEvent");c.initMouseEvent("click",true,true,window,1,f.screenX,f.screenY,f.clientX,f.clientY,false,false,false,false,0,null);
f.target.dispatchEvent(c);if(c.defaultPrevented){if(d.preventDefault){d.preventDefault()
}else{d.returnValue=false}}}a._previousEvent=f}}if(typeof Touch!=="undefined"){document.addEventListener("touchstart",touchHandler,true);
