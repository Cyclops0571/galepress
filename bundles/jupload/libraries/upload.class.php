...
class UploadHandler
{
    ...
    protected function get_unique_filename($name, $type, $index, $content_range) {
        //modified by u@
        //file_put_contents('/vagrant/public/files/temp/fileupload.txt', '1.'.$name, FILE_APPEND);
        ...
        //file_put_contents('/vagrant/public/files/temp/fileupload.txt', '2.'.$name, FILE_APPEND);
        return $name;
    }
	...
	public function gurus_post($print_response = true) {
        if (isset($_REQUEST['_method']) && $_REQUEST['_method'] === 'DELETE') {
            return $this->delete($print_response);
        }
        $upload = isset($_FILES[$this->options['param_name']]) ?
	...
	protected function gurus_handle_file_upload($uploaded_file, $name, $size, $type, $error,
            $index = null, $content_range = null) {
        $file = new stdClass();
        $file->name = $this->get_file_name($name, $type, $index, $content_range);
        $file->size = $this->fix_integer_overflow(intval($size));
